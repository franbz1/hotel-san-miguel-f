...
import { useAnalyticsProcedencia } from "@/hooks/useAnalytics"
...
import { MapPin, Filter, Globe, Users, Info, Trash, Map } from "lucide-react"
import {
...
export function ProcedenciaAnalytics() {
  ...
  const { data, loading, error } = useAnalyticsProcedencia(filtros)
  ...
  // Definir tipo para datos agrupados por país
  interface PaisAgrupado {
    pais: string;
    cantidad: number;
    ciudades: number;
  }
  ...
  const agruparPorPais = (): PaisAgrupado[] => {
    if (!data || data.length === 0) return []
    
    const paisesObj: { [key: string]: PaisAgrupado } = {}
    
    data.forEach(item => {
      const pais = item.paisProcedencia
      if (paisesObj[pais]) {
        paisesObj[pais] = {
          ...paisesObj[pais],
          cantidad: paisesObj[pais].cantidad + item.cantidad,
          ciudades: paisesObj[pais].ciudades + 1
        }
      } else {
        paisesObj[pais] = {
          pais: pais,
    ...
    return Object.values(paisesObj)
      .sort((a: PaisAgrupado, b: PaisAgrupado) => b.cantidad - a.cantidad)
  }

  // Formatear datos para gráfico de pastel (países)
  const formatearDatosPastel = () => {
    const datosPaises = agruparPorPais()
    if (datosPaises.length === 0) return []
    
    const totalHuespedesPastel = datosPaises.reduce((sum: number, item: PaisAgrupado) => sum + item.cantidad, 0)
    
    return datosPaises
      .slice(0, 5) // Top 5 países
      .map((item: PaisAgrupado) => ({
        name: item.pais,
        value: item.cantidad,
        porcentaje: (item.cantidad / totalHuespedesPastel) * 100,
        ciudades: item.ciudades
      }))
  }

  // Formatear datos para gráfico de barras (ciudades)
  const formatearDatosBarras = () => {
    ...
    return data
      ...
      .map(item => ({
        ciudad: item.ciudadProcedencia.length > 12 ? 
          item.ciudadProcedencia.substring(0, 12) + '...' : 
          item.ciudadProcedencia,
        ciudadCompleta: item.ciudadProcedencia,
        pais: item.paisProcedencia,
        cantidad: item.cantidad,
  ...
  const paisesUnicos = agruparPorPais().length
  const ciudadesUnicas = data?.length || 0
  const paisTop = agruparPorPais()[0]
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-white p-3 border border-gray-200 rounded-lg shadow-lg">
          ...
          <p className="text-sm text-orange-600">Ciudades: {data.payload.ciudades}</p>
        </div>
      )
    }
    return null
  ...
  const CustomBarTooltip = ({ active, payload }: {
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-white p-3 border border-gray-200 rounded-lg shadow-lg">
          <p className="font-semibold text-gray-900">{data.payload.ciudadCompleta}</p>
          <p className="text-sm text-gray-600">{data.payload.pais}</p>
          <p className="text-sm text-blue-600">Huéspedes: {data.value}</p>
          <p className="text-sm text-green-600">Porcentaje: {data.payload.porcentaje.toFixed(1)}%</p>
        </div>
      )
      ...
      <div className="space-y-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Configure los parámetros para analizar la procedencia geográfica de huéspedes
            </CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <Tooltip>
                  ...
                  <TooltipContent className="max-w-xs">
                    ...
                    <p className="text-sm">
                      Número total de huéspedes únicos por ubicación geográfica en el período seleccionado.
                      <br/><br/>
                      <strong>Útil para:</strong> Evaluar alcance geográfico del hotel.
                    </p>
                  </TooltipContent>
                </Tooltip>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <Tooltip>
                  ...
                  <TooltipContent className="max-w-xs">
                    ...
                    <p className="text-sm">
                      Diversidad geográfica de mercados internacionales.
                      <br/><br/>
                      <strong>Alta diversidad:</strong> Menos riesgo por dependencia geográfica
                      <br/>
                      <strong>Baja diversidad:</strong> Concentración en pocos mercados
                    </p>
                  </TooltipContent>
                </Tooltip>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <Tooltip>
                  ...
                  <TooltipContent className="max-w-xs">
                    <p className="font-semibold mb-1">¿Qué representa?</p>
                    <p className="text-sm">
                      Número de ciudades únicas de donde provienen los huéspedes.
                      <br/><br/>
                      <strong>Analiza:</strong> Distribución urbana vs. rural y oportunidades de expansión.
                    </p>
                  </TooltipContent>
                </Tooltip>
              </div>
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <div className="flex items-center gap-2">
                <CardTitle className="text-sm font-medium">Principal Origen</CardTitle>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Info className="h-4 w-4 text-muted-foreground cursor-help" />
                  </TooltipTrigger>
                  <TooltipContent className="max-w-xs">
                    ...
                    <p className="text-sm">
                      País con mayor cantidad de huéspedes.
                      <br/><br/>
                      <strong>Analiza:</strong> Dependencia del mercado principal y estrategias de diversificación.
                    </p>
                  </TooltipContent>
                </Tooltip>
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {/* Gráfico de Pastel - Distribución por País */}
          <Card>
            <CardHeader>
              <div className="flex items-center gap-2">
                ...
                <Tooltip>
                  ...
                  <TooltipContent className="max-w-sm">
                    ...
                    <p className="text-sm">
                      Muestra el Top 5 de países por cantidad de huéspedes.
                      <br/><br/>
                      <strong>Segmentos grandes:</strong> Mercados principales para focalizar marketing
                      <br/>
                      <strong>Segmentos pequeños:</strong> Oportunidades de expansión geográfica
                      <br/><br/>
                      <strong>Estrategia:</strong> Diversificar si hay alta concentración en un país
                    </p>
                  </TooltipContent>
                </Tooltip>
              </div>
              <CardDescription>
                Top 5 países por cantidad de huéspedes
              </CardDescription>
            </CardHeader>
            <CardContent>
              {loading ? (
          ...
          <Card>
            <CardHeader>
              <div className="flex items-center gap-2">
                <CardTitle>Top Ciudades de Origen</CardTitle>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Info className="h-4 w-4 text-muted-foreground cursor-help" />
                  </TooltipTrigger>
                  <TooltipContent className="max-w-sm">
                    ...
                    <p className="text-sm">
                      Top 5 ciudades ordenadas por cantidad de huéspedes.
                      <br/><br/>
                      <strong>Barras altas:</strong> Ciudades con mayor demanda hacia el hotel
                      <br/>
                      <strong>Análisis urbano:</strong> Identificar patrones de grandes vs. pequeñas ciudades
                      <br/><br/>
                      <strong>Estrategia:</strong> Enfocar marketing local en ciudades específicas
                    </p>
                  </TooltipContent>
                </Tooltip>
              </div>
              <CardDescription>
                Top 5 ciudades por cantidad de huéspedes
              </CardDescription>
            </CardHeader>
            <CardContent>
              {loading ? (
        ...
        {!loading && data && data.length > 0 && (
          <Card>
            <CardHeader>
              <div className="flex items-center gap-2">
                <CardTitle>Datos Detallados por Ubicación</CardTitle>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Info className="h-4 w-4 text-muted-foreground cursor-help" />
                  </TooltipTrigger>
                  <TooltipContent className="max-w-sm">
                    ...
                    <p className="text-sm">
                      Datos completos para análisis geográfico detallado:
                      <br/><br/>
                      <strong>País:</strong> País de procedencia
                      <br/>
                      <strong>Ciudad:</strong> Ciudad específica de origen
                      <br/>
                      <strong>Cantidad:</strong> Número de huéspedes
                      <br/>
                      <strong>Porcentaje:</strong> Participación del total
                      <br/><br/>
                      <strong>Tip:</strong> Identifica oportunidades de marketing regional
                    </p>
                  </TooltipContent>
                </Tooltip>
              </div>
