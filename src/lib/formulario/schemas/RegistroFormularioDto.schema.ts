...
// Paso 2: Información Personal (incluye datos de reserva + datos personales)
export const informacionPersonalSchema = z.object({
  // Datos de la reserva (solo motivo_viaje es editable por el usuario)
...
// Paso 3: Acompañantes
export const acompaniantesSchema = z.object({
  ...
  huespedes_secundarios: z.array(huespedSecundarioSchema)
    .optional()
    .refine((data) => {
      // Si no hay data, está bien (array vacío)
      if (!data) return true;
      // Validar que la longitud coincida con numero_acompaniantes
      // Esta validación se hará a nivel de formulario completo
      return true;
    })
});
...
export type InformacionPersonalFormData = z.infer<typeof informacionPersonalSchema>;
export type AcompaniantesFormData = z.infer<typeof acompaniantesSchema>;